...
This guide will help you to import the SelfService PowerApp into your own environment, and set-up the requirements that are needed to make this app work. 
...
- Power Apps -> [License FAQ](https://docs.microsoft.com/en-us/power-platform/admin/pricing-billing-skus)
- App registration with Graph API permissions

## Architecture

The concept of this app is very,very simple. It puts users into groups. Those groups live in Azure AD, and can be used within all supported and integrated services, like Azure AD security features, Microsoft Endpoint Manager, or Conditional Access, for example.  

The front-end is this very PowerApp, where the users can add themselves to groups using toggles or sliders. This action will trigger Power Automate to start a flow. This flow will use an HTTP connector to talk to the Graph API, and add/remove the user to/from the group. It uses an app registration to authenticate and authorize the API requests. 
...
![Architecture](images/SelfServicePortal.png)

## Step 1 - Create an app registration and assign permissions

Before we start, we need an app registration in Azure AD.
Assign the following permission for Graph API:

>`https://graph.microsoft.com/Directory.Read.All (application)`
>`https://graph.microsoft.com/Group.ReadWrite.All (application)`

**Don't forget to grant admin consent for the permissions!**

![app registration](images/1641656605.png)

Add a new secret, and capture the value. 

![app secret](images/1641656726.png)

Capture the following information, for later use:
- Application(client) ID
- Directory(tenant) ID
- Secret value

![app registration](images/1641656842.png)
...
> I **strongly** suggest to start off with (empty) test groups, to make sure that all the features of the app work correctly. After testing, you can replace the testgroups with your "production" groups.
...
> TIP! If you just want to explore the app, just create **1** group, and capture the ID. All the features in the app will point to that single group, but for testing and exploring, that's fine. The concept of the app will be the same.
...
![PowerApps - import](images/1641713201.png)
...
![PowerApps - Import screen](images/1641713240.png)
...
> Now, I'm fully aware of the fact that I probably should **not** use the secret as plain text in the flow for security reasons, but I wanted to keep things as easy as possible. I strongly suggest, to take a look at [this blogpost](https://www.inthecloud247.com/use-azure-key-vault-to-secure-your-graph-api-connection-in-power-automate/) to store your secret in an Azure KeyVault. 
...
Let's talk about the slider. The value of the slider will represent group 1, 2 or 3. 
So:
...
If the groupID is not detected in the collection, nothing will happen, and the spinner will stop.

This logic is used for every toggle.
...
1. Create the corresponding group, and capture the groupID for that group. This can also be an existing group. 
2. Add the GroupID to the `GroupIDs` variable in the Power Automate flow. *Don't forget to add the comma!*

![image-addJSON](images/1641745654.png)

3. Define the new feature in the `onStart` property of the Power App. Be careful with messing up the `Concurrent` feature. Because we are adding a new line, you should replace the `));` with `),`

The `));` is now added to your newly added line to close off the array. 

![image-newfeature](images/1641745960.png)

4. Now duplicate one of the existing toggles, and change the `var_featureID` to match with your new added feature number (13 in this example).
